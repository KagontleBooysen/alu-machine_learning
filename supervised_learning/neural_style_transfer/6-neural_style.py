...
class NST:
    ...
    def __init__(self, style_image, content_image, alpha=1e4, beta=1):
        ...
        if (type(style_image) is not np.ndarray or style_image.ndim != 3 or
            ...
            raise TypeError('style_image numpy.ndarray  shape (h, w, 3)')
        if (type(content_image) is not np.ndarray or content_image.ndim != 3 or
                content_image.shape[2] != 3):
            raise TypeError('content_image mus numpy.ndarray  shape (h, w, 3)')
    ...
    def scale_image(image):
        if (type(image) is not np.ndarray or image.ndim != 3 or
                image.shape[2] != 3):
            raise TypeError('image must be a numpy.ndarray shape (h, w, 3)')
        max_dims = 512
        shape = image.shape[:2]
    ...
    def gram_matrix(input_layer):
        if not (isinstance(input_layer, tf.Tensor) or
                isinstance(input_la, tf.Variabl)) or input_la.shape.ndims != 4:
    ...
    def generate_features(self):
        ...
        self.gram_style_features = [
            self.gram_matrix(style_feature) for style_feature in style_feature]

    def layer_style_cost(self, style_output, gram_target):
        if not (isinstance(style_output, tf.Tensor) or
                isinstance(sty_output, tf.Var)) or sty_output.shape.ndims != 4:
        ...
        gram_target.shape.dims != [m, nc, nc]:
            raise TypeError('gram_target  tensor of shape [{}, {}, {}]'.format(
                m, nc, nc))
        gram_style = self.gram_matrix(style_output)
        return tf.reduce_sum(tf.square(gram_style - gram_target)) / tf.square(
                tf.cast(nc, tf.float32))

    def style_cost(self, style_outputs):
        if type(sty_puts) is not list or len(sty_puts) != len(self.sty_layer):
            raise TypeError('style_outputs must be a list length {}'.format(
    ...
    def content_cost(self, content_output):
        ...
        content_output.shape.dims != self.content_feature.shape.dims:
            raise TypeError('content_output must tensor of shape {}'.format(
                self.content_feature.shape))
        _, nh, nw, nc = content_output.shape.dims
        return tf.reduce_sum(tf.square(cont_output - self.cont_feature)) / tf.cast(
            nh * nw * nc, tf.float32)
